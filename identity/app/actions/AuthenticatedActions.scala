    ...
    def refine[A](request: Request[A]) =
      authService.authenticatedUserFor(request) match {
        ...
        case None => {
          ...
          email.map(e => identityApiClient.userFromQueryParam(e, "emailAddress").map { user => {
            user match {
              case Left(userExists)=> Left(sendUserToSignin(request))
              case Right(err) => Left(sendUserToRegister(request))
            }
          }
